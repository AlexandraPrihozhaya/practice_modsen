  ...
  const onRouteBtnClick = async () => {
    try {
      const response = await fetch(
        `https://router.project-osrm.org/route/v1/walking/${userLocation[1]},${userLocation[0]};${object.geometry.coordinates[0]},${object.geometry.coordinates[1]}?overview=full&geometries=geojson`
      );
      ...
      const wayPoints = coordinates.map((oneCordinates) => [
        oneCordinates[1],
        oneCordinates[0],
      ]);
  ...
  const createFavorite = async () => {
    await addDoc(FavoritesCollectionRef, {
      ...
      address: object.properties.CompanyMetaData.address
        ...
        : null,
      hours: object.properties.CompanyMetaData.Hours
        ? object.properties.CompanyMetaData.Hours.text
        : null,
      phone: object.properties.CompanyMetaData.Phones
        ? object.properties.CompanyMetaData.Phones[0].formatted
        : null,
      url: object.properties.CompanyMetaData.url
      ...
      <SCard>
        ...
        {object.properties.CompanyMetaData.url && (
          <>
            <SName>Сайт: </SName>
            <SAddress
              href={object.properties.CompanyMetaData.url}
              target="_blank"
